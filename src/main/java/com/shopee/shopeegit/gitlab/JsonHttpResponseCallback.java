...
import com.shopee.shopeegit.gitlab.exception.GitLabIOException;
import okhttp3.Call;
...
import static com.shopee.shopeegit.gitlab.HttpUtils.assertHasBody;
...
public class JsonHttpResponseCallback<T> implements Callback {
    ...
    private boolean assertNotNullBody(Response response) {
        ...
        if (body == null) {
            result.completeExceptionally(AccessDeniedException.GitLabHttpResponseException.ofNullResponse(response));
            return false;
        }
        return true;
    }
